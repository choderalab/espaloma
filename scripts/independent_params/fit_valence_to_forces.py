...
from espaloma.mm.mm_utils import get_force_targets, MMComponents, initialize_bonds, initialize_angles, initialize_torsions
from espaloma.mm.mm_utils import compute_harmonic_bond_potential, compute_harmonic_angle_potential, compute_periodic_torsion_potential, get_sim
...
if __name__ == '__main__':
    ...
    bond_params = initialize_bonds(sim, offmol)
    angle_params = initialize_angles(sim, offmol)
    torsion_params = initialize_torsions(sim, offmol)
    ...
    langevin_traj, langevin_log_prob_traj = langevin(x0, v0, logprobfun, gradlogprobfun, stepsize=1e-4, collision_rate=1e2, n_steps=10000)
    ...
    plt.savefig(f'plots/{name}_{target_name}_langevin_loss_traj.png', dpi=300)
    plt.close()
