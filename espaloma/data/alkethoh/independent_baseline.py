...
from espaloma.data.alkethoh.mm_utils import get_bond_energy, get_angle_energy, get_torsion_energy
...
def compute_harmonic_bond_potential_alt_param(xyz, params, pair_inds, bond_inds):
    ...
    r = compute_distances(xyz, pair_inds)
    return np.sum(harmonic_bond_potential_alt_param(r, k1, k2, r1, r2), axis=1)
...
def compute_harmonic_angle_potential_alt_param(xyz, params, triple_inds, angle_inds):
    ...
    theta = compute_angles(xyz, triple_inds)
    return np.sum(harmonic_angle_potential_alt_param(theta, k1, k2, theta1, theta2), axis=1)
...
if __name__ == '__main__':
    ...
    valence_target = ani1ccx_energies - nb_target
    ...
    opt_result = basinhopping(fun, x0=opt_result.x, minimizer_kwargs=dict(method='L-BFGS-B', jac=jac), disp=True,
                              stepsize=100.0)
    print(opt_result.x[:n_unique_bonds])
    print(opt_result.x[n_unique_bonds:2 * n_unique_bonds])
