...
from pinot.nn.readout.base_readout import BaseReadout
...
class JanossyPooling(torch.nn.Module):
    ...
    def forward(self, g):
        ...
        g.multi_update_all(
            {
                'n1_as_%s_in_%s' % (
                    ...
                    dgl.function.mean('m%s' % relationship_idx, 'h%s')
        ...
        for big_idx in self.levels:
            g.apply_nodes(
                func=lambda nodes: {
                    'h': getattr(self, 'f_out_%s' % big_idx)(self.pool(
                        torch.cat(
                            [
                        ...
                        torch.cat(
                            [
                                nodes.data['h%s'] % relationship_idx
                                for relationship_idx in range(big_idx, -1, -1)
                            ],
                            dim=1
                        )
                    ))
